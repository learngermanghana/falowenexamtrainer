...
if (!runtimeSource.includes("data-workbook-incomplete-submit-warning")) {
  ...
  runtimeSource = `${runtimeSource.slice(0, effectEndIndex)}${runtimeSource.slice(effectEndIndex, effectEndIndex + '  }, [activeTab, draftContext]);\n\n'.length)}${completionGuard}${runtimeSource.slice(effectEndIndex + '  }, [activeTab, draftContext]);\n\n'.length)}`;
}
