...
if (source.includes(unsafeBlock)) source = source.replaceAll(unsafeBlock, safeBlock);
if (source.includes("submissionContext={routeLockedSubmissionContext}")) throw new Error("Unsafe routeLockedSubmissionContext usage remains in A2 workbook guidance.");
if (!source.includes("submissionContext={resolveA2FallbackSubmissionContext(workbookDay)}")) throw new Error("Safe A2 route-locked submission context was not installed.");
...
await import("./patchC2Days15To21Mastery.mjs");
