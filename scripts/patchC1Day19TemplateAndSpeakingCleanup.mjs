...
const previousDeclaration = `export const C1_OPINION_ESSAY_TEMPLATE = \`${previousTemplateBody}\`;`;
const newDeclaration = `export const C1_OPINION_ESSAY_TEMPLATE = \`${newTemplateBody}\`;`;

if (workspace.includes(previousDeclaration)) {
  workspace = workspace.replace(previousDeclaration, newDeclaration);
}
if (!workspace.includes(newDeclaration)) {
  throw new Error("The Goethe-aligned C1 essay template is missing.");
}

const legacyAnchor = "export const LEGACY_C1_OPINION_ESSAY_TEMPLATES = [";
const serializedPreviousTemplate = JSON.stringify(previousTemplateBody);
if (!workspace.includes(serializedPreviousTemplate)) {
  if (!workspace.includes(legacyAnchor)) {
    throw new Error("The C1 legacy template list was not found.");
  }
...
if (lessonPage.includes(duplicatedQuestion)) {
  lessonPage = lessonPage.replace(duplicatedQuestion, singleQuestion);
}
...
console.log("C1 Day 19 now loads the Goethe template automatically and shows one speaking question.");
