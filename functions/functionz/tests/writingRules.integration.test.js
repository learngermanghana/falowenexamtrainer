...
const signUp = async (prefix) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Auth emulator signup failed: ${response.status} ${await readErrorBody(response)}`,
    );
  }
...
const signInWithCustomToken = async (uid, claims = {}) => {
  ...
  const serviceAccount = `firebase-adminsdk@${PROJECT_ID}.iam.gserviceaccount.com`;
  ...
  const response = await fetch(
    `${AUTH_URL}/accounts:signInWithCustomToken?key=${API_KEY}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  if (!response.ok) {
    throw new Error(
      `Custom token signin failed: ${response.status} ${await readErrorBody(response)}`,
    );
  }
  return response.json();
};
...
const ownedWritingCases = [
  ...
  [
    "writingProgress attempt",
    (uid) => `writingProgress/${uid}__course/attempts/attempt-1`,
  ],
...
describe("private writing Firestore rules", () => {
  ...
  test.each(ownedWritingCases)(
    "%s allows only the owner to create/read/delete",
    async (_label, pathFor) => {
      ...
      expect(
        (
          await write(other.idToken, pathFor(other.localId), {
            ...attempt,
            userId: owner.localId,
          })
        ).status,
  ...
  it("allows staff reads through explicit admin/tutor claims without granting student writes", async () => {
    ...
    const staff = await signInWithCustomToken(`staff-${Date.now()}`, {
      role: "tutor",
    });
    ...
    expect(
      (
        await write(staff.idToken, path, {
          userId: owner.localId,
          topic: "Changed by staff",
        })
      ).status,
    ).toBe(403);
  });
