...
const validateLessonCatalog = (inputLessons = lessonCatalog) => {
  ...
  inputLessons.forEach((lesson) => {
    ...
    if (lesson.submissionRequired && !lesson.workbookRoute && !lesson.grammarPage) errors.push(`Required lesson has no workbookRoute or grammarPage: ${lesson.id}`);
  });
...
const CURRICULUM_ENTRIES = lessonCatalog.map(toLegacyCurriculumEntry);
