...
const A2_SCHEDULE = [
  {
    ...
    instruction:
      "Start here before Day 1. Open the Day 0 workbook first to follow the tutorial and course workflow. Then complete the starter multiple-choice check.",
  ...
  },
  {
    ...
    youtube_link: "https://youtu.be/DfJ04x4JGOo",
...
const buildDictionaryBackedSchedule = (level) =>
  getCurriculumEntriesForLevel(level).map((entry) => ({
    ...
    lesen_hören: {
      chapter: entry.chapter || null,
      assignment: entry.assignment === true,
      video: level === "C1" && Number(entry.assignmentDay) === 1 ? "https://youtu.be/McNk1VTFvMk" : "",
      youtube_link: level === "C1" && Number(entry.assignmentDay) === 1 ? "https://youtu.be/McNk1VTFvMk" : "",
