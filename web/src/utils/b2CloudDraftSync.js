...
export const shouldPreserveNewerLocalB2Draft = ({
  remoteSerialized,
  localSerialized,
  dirtySerialized,
  isOwnWrite,
...
export const buildB2CloudDraftDocId = (day) => `day-${Number(day)}`;

export const useB2CloudDraftField = ({ day, field, value, setValue, seedCloudWhenMissing = false, defaultValue }) => {
  const { user } = useAuth();
  ...
  const writerIdRef = useRef(
    typeof globalThis !== "undefined" && typeof globalThis.crypto?.randomUUID === "function"
      ...
      : `b2-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  const migrationKey = user?.uid && day && field
    ? `falowen:b2:cloud-migrated:${user.uid}:${Number(day)}:${field}`
    : "";
  ...
  const flushPendingSave = useCallback(() => {
    ...
    setDoc(pending.draftRef, pending.payload, { merge: true })
      ...
      .catch((error) => {
        console.error(`B2 cloud draft save failed for ${field}`, error);
      });
  }, [field, migrationKey]);
  ...
  useEffect(() => {
    ...
    const draftRef = doc(db, "users", user.uid, "b2Drafts", buildB2CloudDraftDocId(day));
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (hasRemoteField) {
          ...
          const shouldPreserveLocal = shouldPreserveNewerLocalB2Draft({
            remoteSerialized,
            localSerialized,
      ...
      (error) => {
        console.error(`B2 cloud draft load failed for ${field}`, error);
