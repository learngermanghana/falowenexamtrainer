...
// Articleless case correction is only safe when the noun's grammatical number
// and case form are known. Keep this lexicon deliberately small and explicit.
...
const adjectiveStem = (word) => {
  ...
  if (/(e|er)$/u.test(lower)) return lower.replace(/(?:e|er)$/u, "");
  return lower;
};
...
const stableId = (signature) => {
  ...
  return `preposition-case-articleless-${(hash >>> 0).toString(36)}`;
};
...
const analyzeArticlelessKnownNouns = (text) => {
  ...
  for (let index = 0; index < tokens.length - 2; index += 1) {
    ...
    issues.push({
      ...
      explanation: `“${nounInfo.display}” is plural here. After “${preposition.value}”, use ${grammaticalCase}: “${fullCorrection}”.`,
