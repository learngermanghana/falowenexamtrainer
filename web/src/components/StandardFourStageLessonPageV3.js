...
} from "./selfLearning/EmbeddedPracticePanels";
...
import C1Day3SpeakingMindMap from "./C1Day3SpeakingMindMap";
...
const matches = (lesson, level, day) =>
  String(lesson?.level || "").toUpperCase() === level &&
...
const isC1Day3 = (lesson) => matches(lesson, "C1", 3);
  ...
  <div style={{ display: "grid", gap: 12 }}>
    ...
    >
      ...
      <p style={{ margin: "0 0 8px", color: "#475569" }}>
        Wähle passende Punkte aus und gib Gründe und Beispiele.
      </p>
      <ul style={listStyle}>
        {branches.map((branch) => (
...
const SpeakingBuilder = ({ lesson }) => {
  ...
  if (isC1Day3(lesson)) return <C1Day3SpeakingMindMap lesson={lesson} />;
  if (isCompactC1(lesson)) {
    const branches = isC1Day2(lesson)
      ? c1Day2LearningSpeakingGuide.speaking.branches
      : rich;
    return (
      <CompactSpeakingPoints
        question={c1Questions[Number(lesson.day)]}
        branches={branches}
      />
    );
  }
      ...
      <div style={{ display: "grid", gap: 12 }}>
        <div>
          ...
          <p style={{ margin: 0, color: "#475569" }}>
            Wähle passende Themenbereiche aus, verknüpfe sie differenziert und
            entwickle daraus deine eigene Antwort.
          </p>
        </div>
        {rich.map((branch, index) => (
          ...
          >
            ...
            <div>
              <strong>Leitfrage:</strong> {branch.prompt}
            </div>
            <div style={{ color: "#3730a3" }}>• {branch.example}</div>
            {branch.starter ? (
              <div
                style={{ borderLeft: "4px solid #818cf8", paddingLeft: 10 }}
              >
                <strong>Fortgeschrittener Satzanfang:</strong> {branch.starter}
              </div>
            ) : null}
          </article>
        ))}
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      >
        <div>
          <small style={{ opacity: 0.82, fontWeight: 800 }}>
            SPRECHEN-BRAIN-MAP
          </small>
          <h3 style={{ margin: "5px 0 0" }}>{lesson.title}</h3>
        </div>
      </div>
      <div
        style={{
        ...
        {branches.map((branch, index) => (
          <article
            ...
            style={{
              ...
              borderRadius: 16,
              padding: 14,
              background: index % 2 ? "#f8fafc" : "#eef2ff",
              display: "grid",
              gap: 8,
            }}
          >
      ...
      <NoteBox tone="amber">
        <strong>Speaking order:</strong> Follow the branches from 1 to{" "}
        {branches.length}. Give reasons and at least one concrete example.
      </NoteBox>
    </div>
  );
};

export const shouldShowStandardRadioGate = (falowenRadio) =>
  Boolean(falowenRadio);

export default function StandardFourStageLessonPage({
  lesson,
  ...
  const [entered, setEntered] = useState(() =>
    !shouldShowStandardRadioGate(radio),
  );
    ...
    return (
      <div style={{ ...styles.container, display: "grid", gap: 18 }}>
        ...
        >
          ...
          <h1 style={{ margin: 0 }}>
            {lesson.level} · Day {lesson.day} · {lesson.title}
          </h1>
          <p style={{ margin: 0, color: "#475569" }}>
            Listen to Falowen Radio first. Continue opens Learn, Speak, Write and
            Finish.
          </p>
        </header>
  ...
  const grammarUrl =
    canonicalLesson?.resources?.grammarBook?.url ||
    lesson.resources?.grammarBook?.url ||
    "";
  const workbookUrl =
    ...
    "";
  const finishReady =
    progress.learnDone && progress.speakDone && writing.complete;
  const assignmentId = canonicalLesson?.submission?.assignmentId;
  const canSubmit = Boolean(
    canonicalLesson?.submission?.enabled && assignmentId,
  );
  const fullEssay =
    getAdvancedWritingPhase(lesson.level, lesson.day) === "full-essay";
  ...
  const finish = () => {
    ...
    showToast(
      `${lesson.level} Day ${lesson.day} completed. Your progress was saved.`,
      "success",
    );
  };
  ...
  return (
    <div style={{ ...styles.container, display: "grid", gap: 18 }}>
      ...
      >
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: "clamp(2rem,5vw,3.6rem)" }}>
            {lesson.title}
          </h1>
          ...
          <Section title="AI video">
            {video?.url ? (
              ...
              <NoteBox tone="amber">
                ...
                grammar notes and lesson resources.
              </NoteBox>
            )}
          </Section>
          <Section
            title={`Grammar: ${lesson.grammarLesson?.title || lesson.grammarFocus || lesson.title}`}
          >
            <NoteBox tone="amber">
              <strong>Focus:</strong>{" "}
              {lesson.grammarFocus ||
                lesson.grammarLesson?.title ||
                lesson.title}
            </NoteBox>
            <div
              style={{
            ...
            <ResourceButton href={grammarUrl}>
              Open full grammar notes
            </ResourceButton>
      ...
      {active === "write" ? (
        <Section title="Guided writing builder">
          <NoteBox>
            <strong>Task:</strong>{" "}
            {lesson.writingTopic ||
              `Schreibe einen Text zum Thema „${lesson.title}“.`}
          </NoteBox>
          <ResourceButton href={workbookUrl}>
      ...
      {active === "references" ? (
        <WorkbookReferenceAnswers
          level={lesson.level}
          lesson={lesson}
          workbookId={`${lesson.level}-day-${lesson.day}`}
        />
      ) : null}
        ...
        <Section title={`Finish ${lesson.level} Day ${lesson.day}`}>
          ...
          >
            <ProgressCard
              ...
              detail="Video and grammar reviewed"
            />
            <ProgressCard
              label="Speak"
              ...
              detail={
                isCompactC1(lesson)
                  ...
                  : "Brain-map speaking practice completed"
              }
            />
            <ProgressCard
              label="Write"
              complete={writing.complete}
              detail={`${writing.completedQuestions}/${writing.totalQuestions} questions · ${writing.wordCount} final words`}
            />
          </div>
          <label style={{ display: "grid", gap: 7 }}>
            <strong>Short reflection</strong>
            <span style={{ color: "#64748b", fontSize: 13 }}>
              What did you learn, and what should you improve next?
            </span>
            <textarea
          ...
          {!finishReady ? (
            <NoteBox tone="amber">
              Complete Learn, Speak and the guided writing task before finishing
              this lesson.
            </NoteBox>
          ) : null}
          {progress.completed ? (
            <NoteBox tone="green">
              <strong>Completed.</strong> This lesson is saved as complete on this
              device.
            </NoteBox>
          ) : null}
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button
              ...
              onClick={finish}
            >
              {progress.completed ? "Mark complete again" : "Mark lesson complete"}
            </button>
            {canSubmit ? (
              <button
                type="button"
