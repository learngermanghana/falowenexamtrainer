...
export default function B2Day7To16GuidedLessonPage({ lesson, canonicalLesson = null }) {
  ...
  const [progress, setProgress] = useState(() => {
    try {
      return {
        ...
        speakDone: false,
        completed: false,
        ...JSON.parse(localStorage.getItem(getStandardLessonStorageKey(lesson, "progress")) || "{}"),
      };
    } catch {
  ...
  return (
    <div style={{ ...styles.container, display: "grid", gap: 18 }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 5, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 8, padding: 10, border: "1px solid #e2e8f0", borderRadius: 18, background: "rgba(248,250,252,.94)" }}>
        {tabs.map((tab) => (
          <button
            ...
            style={{ ...(active === tab ? styles.primaryButton : styles.secondaryButton), borderRadius: 999, minHeight: 44 }}
          >
            {labels[tab]}
          </button>
        ))}
              ...
              <div style={{ display: "grid", gap: 10 }}>
                ...
                {videoEmbed ? (
                  <div style={{ position: "relative", width: "100%", paddingTop: "56.25%", borderRadius: 16, overflow: "hidden", background: "#0f172a" }}>
                    <iframe
                      title={video.title || "B2 lesson video"}
                      src={videoEmbed}
        ...
        <Section title="Speaking builder">
          ...
          <label style={{ display: "flex", gap: 9, alignItems: "center", fontWeight: 800 }}>
            <input
              ...
              onChange={(event) => setProgress((old) => ({ ...old, speakDone: event.target.checked }))}
            />
      ...
      {active === "references" ? (
        <WorkbookReferenceAnswers level="B2" lesson={lesson} workbookId={`B2-day-${day}`} />
      ) : null}
