...
const A2StandardTabbedWorkbookPage = ({ day, title, chapter, topicPrompt, workbookId, sprechenContent, showSpeakingTaskCard = true, mindMapOnlySpeaking = false, schreibenTask, schreibenContent, schreibenPlaceholder = "Liebe/r ...\n\nich schreibe, weil ...", lesenText, lesenQuestions = [], hoerenTask, hoerenAudioUrl, hoerenQuestions = [], showHoeren = true, showWorkbookGuidance = true, openGrammarAfterRadio = false }) => {
  const location = useLocation();
  const radioCompleted = useMemo(
    () => openGrammarAfterRadio && new URLSearchParams(location.search || "").get("radio") === "done",
    [openGrammarAfterRadio, location.search],
  );
  ...
  return <div style={{ ...styles.container, display: "grid", gap: 16 }}>
    <div style={card}>
      ...
      <p style={{ ...styles.subtitle, margin: 0 }}>{radioCompleted ? "Falowen Radio complete — continue with Grammar, then work through the remaining sections." : (showHoeren ? "Select Grammar, Teil 1–4, Ref or Submit below." : "Select Grammar, Teil 1–3, Ref or Submit below.")} The tabs stay visible at the top of the workbook.</p>
