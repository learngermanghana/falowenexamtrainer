...
const emptyState = () => ({ answers: {}, finalEssay: "", combinedDraftMode: "auto", analysisFeedback: "", analysisUpdatedAt: "", updatedAt: "" });
...
export default function GuidedWritingWorkspace({ config, storageKey, cloudField, onStatusChange }) {
  ...
  const analyse = async () => {
    ...
    try {
      ...
      update((old) => ({ ...old, analysisFeedback: data?.feedback || data?.structuredFeedback?.summary || "Analysis completed.", analysisUpdatedAt: new Date().toISOString() }));
