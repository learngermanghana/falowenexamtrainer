  ...
  return (
    <section data-writing-history style={{ ...styles.helperCard, display: "grid", gap: 12 }}>
      ...
      {!entries.length ? <p style={{ margin: 0 }}>No saved writing attempts yet.</p> : entries.slice().reverse().map((entry, index) => {
        const entryKey = entry.id || `${entry.submissionDate}-${index}`;
        const isFeedbackExpanded = Boolean(expandedFeedback[entryKey]);

        return (
          <article key={entryKey} style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 12, background: "#fff", display: "grid", gap: 10 }}>
              ...
              <button
                ...
                onClick={() =>
                  setExpandedFeedback((current) => ({
                    ...current,
                    [entryKey]: !isFeedbackExpanded,
                  }))
                }
              >
                {isFeedbackExpanded ? "Hide saved AI feedback" : "Show saved AI feedback"}
              </button>
              {isFeedbackExpanded ? (
