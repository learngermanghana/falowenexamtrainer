...
const lesson = {
  ...
  grammarLesson: {
    ...
    knowledgeTest: [
      { question: "Welche Formulierung ist korrekt?", options: ["deutlich vielfältiger als", "mehr vielfältiger als", "vielfältiger wie"], answer: "deutlich vielfältiger als", explanation: "Nach dem Komparativ steht als." },
      { question: "Welche Struktur drückt Gleichheit aus?", options: ["so ... wie", "so ... als", "mehr ... wie"], answer: "so ... wie", explanation: "so ... wie drückt Gleichheit aus." },
    ],
  },
};

describe("C1KnowledgeChoicePractice", () => {
  beforeEach(() => { window.localStorage.clear(); window.scrollTo = jest.fn(); });

  test("limits the interactive Learn flow to C1 days 1 through 16", () => {
    ...
    expect(getC1KnowledgeItems({ ...lesson, day: 16 })).toHaveLength(2);
    expect(getC1KnowledgeItems({ ...lesson, day: 17 })).toHaveLength(0);
