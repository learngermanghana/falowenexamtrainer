...
import { enhanceB2Day16To20Lesson } from "../data/b2Days16To20SelfTutoring";
...
const GrammarNotes = ({ day, checked, onCheckedChange }) => day === 16
  ? <B2Day14To16GrammarNotes day={day} checked={checked} onCheckedChange={onCheckedChange} />
  : <B2Day17To20GrammarNotes day={day} checked={checked} onCheckedChange={onCheckedChange} />;

export default function B2Day16To20SelfTutoringPage({ lesson, canonicalLesson = null }) {
  const { showToast } = useToast();
  const guidedLesson = useMemo(() => enhanceB2Day16To20Lesson(lesson), [lesson]);
  ...
  return <div style={{ ...styles.container, display:"grid", gap:18 }}>
    ...
    {active === "learn" ? <>
      ...
      <Section title="Deep grammar notes"><GrammarNotes day={day} checked={progress.learnNotesDone} onCheckedChange={(checked)=>setProgress((old)=>({...old,learnNotesDone:checked}))} /></Section>
    </> : null}
    ...
    {active === "finish" ? <Section title={`Summary B2 Day ${day}`}><NoteBox tone={progress.learnNotesDone && progress.quizDone && progress.speakDone ? "green" : "amber"}><strong>Progress:</strong> Grammar notes {progress.learnNotesDone?"✓":"—"} · Quiz {progress.quizDone?"✓":"—"} · Speaking {progress.speakDone?"✓":"—"}</NoteBox>{progress.completed ? <NoteBox tone="green"><strong>Completed.</strong> This lesson is saved as complete on this device.</NoteBox> : null}<button type="button" style={{...styles.primaryButton,width:"fit-content"}} onClick={finish}>I have completed</button></Section> : null}
  </div>;
}
