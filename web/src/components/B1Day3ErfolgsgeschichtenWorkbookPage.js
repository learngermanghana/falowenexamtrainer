...
const B1Day3PreservedSections = ({ activeTab, prepared, setPreparedFor }) => {
  const reading = getB1ReadingTask(3);
...
const config = {
  day: 3,
  chapter: "1.3",
  assignmentKey: "B1-1.3",
  workbookId: "B1Day3Erfolgsgeschichten",
  title: "Erfolgsgeschichten",
  subtitle: "Select Grammar, Teil 1–4, Ref or Submit. The existing Day 3 assignments are preserved inside the shared B1 workbook shell.",
  submitListening: true,
  listening: { submitRequired: true },
};

export default function B1Day3ErfolgsgeschichtenWorkbookPage() {
  return <B1StandardWorkbookPage config={config} renderSections={B1Day3PreservedSections} />;
}
