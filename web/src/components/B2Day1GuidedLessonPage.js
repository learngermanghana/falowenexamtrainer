...
export default function B2Day1GuidedLessonPage({ lesson, canonicalLesson = null }) {
    ...
    return (
      <div style={{ ...styles.container, display: "grid", gap: 18 }}>
        ...
        <header style={{ ...card, borderColor: "#bfdbfe", background: "linear-gradient(135deg,#eff6ff,#f8fafc)" }}>
          ...
          <h1 style={{ margin: 0 }}>B2 · Day 1 · {lesson.title}</h1>
          <p style={{ margin: 0, color: "#475569" }}>Listen to Falowen Radio first. Continue opens Learn, Speak, Write and Finish.</p>
        </header>
        <FalowenRadioTabContent level="B2" day={1} resource={radio} onContinue={() => { setEntered(true); window.scrollTo({ top: 0, behavior: "smooth" }); }} />
      </div>
    );
  }
  ...
  const submit = () => {
    ...
    navigate(`/campus/course?submitWork=1&assignmentKey=${encodeURIComponent(assignmentId)}&assignmentId=${encodeURIComponent(assignmentId)}`, {
      state: { assignmentKey: assignmentId, assignmentId, canonicalAssignmentId: assignmentId, day: 1, level: "B2", assignmentTitle: lesson.title },
    });
  };
  ...
  return (
    <div style={{ ...styles.container, display: "grid", gap: 18 }}>
      ...
      {active === "learn" ? <>
        ...
        <B2Day1IdentityGrammarNotes checked={progress.learnDone} onCheckedChange={(checked) => setProgress((old) => ({ ...old, learnDone: checked }))} />
      </> : null}
      ...
      {active === "write" ? <Section title="Guided writing builder">
        <WritingCheatSheetTabs level="B2" day={1}>
      ...
      {active === "references" ? <WorkbookReferenceAnswers level="B2" lesson={lesson} workbookId="B2-day-1" /> : null}

      {active === "finish" ? <Section title="Finish B2 Day 1">
