...
test("recording-only submission continues chat before completion", async () => {
  ...
  sendSpeechTrainerAttempt.mockResolvedValueOnce({ transcript: "short", feedback: "Quick tip" });
  ...
  expect(screen.getByText(/speechTrainer.audioFeedbackLabel: Quick tip/)).toBeInTheDocument();
});
