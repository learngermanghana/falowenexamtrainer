...
const C2Day8To14MasteryPage=({lesson})=>{
 const day=Number(lesson?.day||0);
 if(day>=8&&day<=11)return <><C2Days6To11GuidedWorkbookPage lesson={lesson}/><C2AlignedTeachingSummary lesson={lesson}/></>;
 const d=lesson?.c2Mastery||getC2Day8To14Mastery(day);
 if(!d)return null;
 const card={...styles.card,display:"grid",gap:10};
 return <><main style={{...styles.container,display:"grid",gap:14}} data-c2-mastery-day={day}>
  <header style={card}><strong>C2 · Day {day} · Chapter {d.chapter}</strong><h1 style={{...styles.title,margin:0}}>{d.title}</h1><p>{d.topic}</p><strong>{d.grammarFocus}</strong><p>C2 focus: precision, register, nuance, idiomaticity and stylistic control.</p></header>
  ...
  <section style={card}><h2>Grammar & Style Contrast</h2>{d.contrast.map((x,i)=><p key={x}><strong>Version {i+1}:</strong> {x}</p>)}</section>
...
export default C2Day8To14MasteryPage;
