...
import { getDiscussionLessonsForLevel } from "../utils/discussionLessons";
...
export const ClassDiscussionPanel = ({ embedded = false, lessonId, lessonLabel, title = "Class discussion", description }) => {
  ...
  const lessonOptions = useMemo(() => {
    const lessons = getDiscussionLessonsForLevel(studentProfile?.level);
    if (!embedded || !lessonId) return lessons;
    const match = lessons.find((lesson) => lesson.id === lessonId);
    return match ? [{ ...match, label: lessonLabel || match.label }] : [{ id: lessonId, label: lessonLabel || lessonId, level: studentProfile?.level }];
  }, [embedded, lessonId, lessonLabel, studentProfile?.level]);
  ...
  const filteredThreadsWithReplies = useMemo(() => {
    ...
    const filtered = threadsWithReplies.filter((thread) => {
      ...
      if (embedded && thread.lessonId !== lessonId) return false;
      if (!embedded && lessonFilter !== "all" && thread.lessonId !== lessonFilter) return false;
  ...
  }, [embedded, lessonId, lessonFilter, searchTerm, showMyPostsOnly, showNoRepliesOnly, sortBy, statusFilter, threadsWithReplies, user?.uid]);
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <div style={styles.card}>
        ...
        >
          <div>
            ...
            <p style={{ ...styles.helperText, marginBottom: 0 }}>
              {description || "Anyone in your class can read and reply. Replies update live, and you can edit your own posts if you make mistakes."}
            </p>
        ...
        {!embedded ? <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
          <button
        ...
        {activeTab === "discussion" && (!embedded || isDiscussionAdmin) ? (
          <form onSubmit={handleCreateThread} style={{ display: "grid", gap: 10, marginTop: 12 }}>
            ...
            <div style={{ display: "grid", gap: 10, gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" }}>
              {!embedded ? <div style={styles.field}>
      ...
      {activeTab === "discussion" ? (
        <div style={{ display: "grid", gap: 12 }}>
          {!embedded ? <div style={{ ...styles.card, display: "grid", gap: 10 }}>
