...
const GrammarNotes = ({ day, checked, onCheckedChange }) => day >= 1 && day <= 7 ? <C1Day1To7GrammarNotes day={day} checked={checked} onCheckedChange={onCheckedChange} /> : Number(day) === 11 ? <C1Day11GrammarNotes checked={checked} onCheckedChange={onCheckedChange} /> : <C1Day8To10GrammarNotes day={day} checked={checked} onCheckedChange={onCheckedChange} />;

const SpeakingBuilder = ({ lesson }) => {
  const branches = lesson.speakingBuilder?.branches || [];
  const question = String(lesson.speakingBuilder?.question || lesson.speakingTopic || lesson.topic || "").replace(/^Sprechen:\s*/i, "");
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <div style={{ border: "1px solid #c7d2fe", borderRadius: 14, padding: 14, background: "#eef2ff" }}>
        ...
        <ul style={listStyle}>
          {branches.map((branch) => <li key={branch.id || branch.title}><strong>{branch.title}:</strong> {(branch.keywords || []).join(", ")}</li>)}
        </ul>
      </div>
    </div>
  );
