...
const summaries = {
  1: { title: "Ziele und Lernweg: C1 bewusst planen", intro: "C1 Day 1 hilft Lernenden, ihren Lernweg bewusst zu strukturieren. Es geht darum, Ziele nicht nur zu nennen, sondern realistisch zu begründen und den eigenen Fortschritt messbar zu machen.", points: ["Formuliere konkrete Ziele statt allgemeiner Wünsche.", "Reflektiere, welche Routinen und Strategien deinen Fortschritt sichern.", "Eine starke C1-Antwort erklärt Ziel, Grund, Methode und erwartete Wirkung."], vocabulary: ["die Zielsetzung", "der Lernweg", "die Reflexion", "messbarer Fortschritt", "Eigenverantwortung"] },
  2: { title: "Kultur und Identität: Zugehörigkeit differenziert beschreiben", intro: "C1 Day 2 betrachtet Identität als Zusammenspiel von Herkunft, Sprache, Erfahrungen und gesellschaftlichen Erwartungen. Ziel ist es, Zugehörigkeit nicht vereinfacht, sondern vielschichtig zu erklären.", points: ["Beschreibe Identität als dynamischen Prozess.", "Nutze Relativsätze und Nominalisierungen, um Zusammenhänge zu verdichten.", "Eine starke C1-Antwort vermeidet Klischees und erkennt Mehrfachzugehörigkeit an."], vocabulary: ["die Zugehörigkeit", "die Herkunft", "kulturelle Prägung", "das Selbstbild", "Mehrfachzugehörigkeit"] },
  3: { title: "Medien und Informationskompetenz: Quellen kritisch prüfen", intro: "C1 Day 3 fokussiert Fake News, digitale Quellen und den bewussten Umgang mit Informationen. Wichtig ist nicht nur, was gesagt wird, sondern wer es sagt, mit welchem Beleg und in welchem Kontext.", points: ["Prüfe Quelle, Datum, Autorenschaft und Absicht einer Information.", "Nutze indirekte Rede und Distanzmarker, um Aussagen sachlich wiederzugeben.", "Eine starke C1-Antwort bewertet Informationen, ohne vorschnell zu urteilen."], vocabulary: ["die Quelle", "die Glaubwürdigkeit", "die Autorenschaft", "angeblich", "kritisch einordnen"] },
  4: { title: "Beziehungen und Teamarbeit: Konflikte konstruktiv analysieren", intro: "C1 Day 4 zeigt, dass Beziehungen und Teamarbeit nicht nur Harmonie bedeuten. Konflikte, Rollen, Kommunikation und Verantwortung müssen differenziert betrachtet werden.", points: ["Analysiere, warum Konflikte entstehen und wie sie gelöst werden können.", "Nutze konzessive Strukturen, um Spannungen fair darzustellen.", "Eine starke C1-Antwort erklärt sowohl individuelle als auch strukturelle Faktoren."], vocabulary: ["die Zusammenarbeit", "die Kommunikationskultur", "Rollen klären", "Konflikte lösen", "Verantwortung teilen"] },
  5: { title: "Berufliche Entwicklung: Kompetenzen langfristig ausbauen", intro: "C1 Day 5 behandelt Arbeit, Karriere und Weiterbildung als dynamischen Prozess. In einer sich verändernden Arbeitswelt müssen Kompetenzen regelmäßig erweitert werden.", points: ["Beschreibe Karriere nicht nur als Aufstieg, sondern als Entwicklung von Kompetenzen.", "Nutze Modalpassiv und Nominalstil, um berufliche Prozesse sachlich zu analysieren.", "Eine starke C1-Antwort verbindet Eigeninitiative mit strukturellen Chancen."], vocabulary: ["die Weiterbildung", "berufliche Mobilität", "Kompetenzen erweitern", "Arbeitsmarkt", "Eigeninitiative"] },
  6: { title: "Gesundheit und Lebensstil: Ursachen und Folgen verbinden", intro: "C1 Day 6 analysiert Gesundheit als Zusammenspiel von Lebensstil, Arbeit, Prävention und sozialem Umfeld. Der Fokus liegt auf Ursache, Folge und realistischen Maßnahmen.", points: ["Erkläre Gesundheitsverhalten nicht nur individuell, sondern auch gesellschaftlich.", "Nutze kausale und konsekutive Strukturen für klare Argumentation.", "Eine starke C1-Antwort nennt Ursachen, Folgen und präventive Lösungen."], vocabulary: ["die Prävention", "die Belastung", "aufgrund", "folglich", "Gesundheitsförderung"] },
  7: { title: "Reisen und Nachhaltigkeit: Mobilität verantwortungsvoll bewerten", intro: "C1 Day 7 verbindet Reisen mit ökologischer und sozialer Verantwortung. Es geht darum, Mobilität differenziert zu vergleichen und nachhaltige Alternativen realistisch zu bewerten.", points: ["Vergleiche Reiseformen nach Kosten, Zeit, Komfort und Emissionen.", "Nutze erweiterte Vergleichsformen und abwägende Strukturen.", "Eine starke C1-Antwort fordert nicht nur Verzicht, sondern bewusstere Mobilität."], vocabulary: ["die Mobilität", "nachhaltiges Reisen", "Emissionen", "der Massentourismus", "bewusster Umgang"] },
};

const embedUrl = (url = "") => {
  try { const parsed = new URL(url); const host = parsed.hostname.replace(/^www\./, ""); const id = host === "youtu.be" ? parsed.pathname.replace(/^\//, "") : parsed.searchParams.get("v") || parsed.pathname.split("/").filter(Boolean).pop(); return id ? `https://www.youtube.com/embed/${id}` : ""; } catch { return ""; }
};

const FinishSummary = ({ day }) => {
  const item = summaries[day];
  if (!item) return null;
  return <div style={{ border: "1px solid #bfdbfe", borderRadius: 16, padding: 14, background: "#eff6ff", display: "grid", gap: 10 }}><h3 style={{ margin: 0 }}>{item.title}</h3><p style={{ margin: 0, lineHeight: 1.7 }}>{item.intro}</p><ul style={listStyle}>{item.points.map((point) => <li key={point}>{point}</li>)}</ul><div><strong>Wortschatz:</strong> {item.vocabulary.join(" · ")}</div></div>;
};

export default function C1Day1To7GuidedLessonPage({ lesson, canonicalLesson = null }) {
  ...
  const [progress, setProgress] = useState(() => {
    try { return { learnDone: false, speakDone: false, completed: false, ...JSON.parse(localStorage.getItem(storageKey) || "{}") }; } catch { return { learnDone: false, speakDone: false, completed: false }; }
  });
