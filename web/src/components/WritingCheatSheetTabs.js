  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <div role="tablist" aria-label="Writing support" style={{ display: "flex", gap: 8, flexWrap: "wrap", padding: 6, border: "1px solid #dbeafe", borderRadius: 14, background: "#eff6ff" }}>
        <button id={taskTabId} type="button" role="tab" aria-selected={writeView === "task"} aria-controls={taskPanelId} onClick={() => setWriteView("task")} style={{ ...(writeView === "task" ? styles.primaryButton : styles.secondaryButton), borderRadius: 999 }}>Schreiben Task</button>
        <button id={cheatSheetTabId} type="button" role="tab" aria-selected={writeView === "cheatSheet"} aria-controls={cheatSheetPanelId} onClick={() => setWriteView("cheatSheet")} style={{ ...(writeView === "cheatSheet" ? styles.primaryButton : styles.secondaryButton), borderRadius: 999 }}>Cheat Sheet</button>
      </div>

      <div id={taskPanelId} role="tabpanel" hidden={writeView !== "task"} aria-labelledby={taskTabId}>{children}</div>

      {writeView === "cheatSheet" ? (
        <div id={cheatSheetPanelId} role="tabpanel" aria-labelledby={cheatSheetTabId} style={{ display: "grid", gap: 16 }}>
