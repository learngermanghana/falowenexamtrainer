...
export default function B2Day11To15SelfTutoringPage({ lesson, canonicalLesson = null }) {
  ...
  return <div style={{ ...styles.container, display:"grid", gap:18 }}>
    ...
    {active === "learn" ? <><Section title="AI video">{videoEmbed ? <iframe title={video?.title || "B2 lesson video"} src={videoEmbed} allowFullScreen style={{ width:"100%", minHeight:360, border:0, borderRadius:14 }} /> : <NoteBox tone="amber">Continue with the quick grammar preview and clickable check.</NoteBox>}</Section><B2TopicIntroduction day={day} /><B2QuizFirstLearnPreview lesson={guidedLesson} /><B2KnowledgeChoicePractice lesson={guidedLesson} onCompleteChange={(quizDone) => setProgress((old) => ({ ...old, quizDone }))} /><Section title="Deep grammar notes"><GrammarNotes day={day} checked={progress.learnNotesDone} onCheckedChange={(learnNotesDone) => setProgress((old) => ({ ...old, learnNotesDone }))} /></Section></> : null}
