...
export default function C1KnowledgeChoicePractice({ lesson, completed = false, onCompleteChange }) {
  ...
  if (!items.length) return <><C1SpeakGrammarGuide lesson={lesson} showGrammar showSpeaking={false} /><section style={shellStyle}><h2 style={{ margin: 0 }}>Knowledge practice</h2><p style={{ margin: 0 }}>Multiple-choice knowledge questions are not available for this lesson yet.</p></section></>;
  ...
  return <><C1SpeakGrammarGuide lesson={lesson} showGrammar showSpeaking={false} /><section style={shellStyle} aria-label={`C1 Day ${lesson.day} knowledge practice`}>
