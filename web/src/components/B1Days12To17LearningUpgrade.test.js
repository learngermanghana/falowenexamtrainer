...
describe("B1 Days 12-17 thinking and quiz-first grammar upgrade", () => {
  test("central grammar tab shows the range upgrade before deep grammar notes", () => {
    ...
    expect(source).toContain('normalizedLevel === "B1" && numericDay >= 12 && numericDay <= 17');
    expect(source.indexOf("<B1Days12To17LearningUpgrade day={numericDay} />")).toBeLessThan(
      source.indexOf("<GrammarNotes />"),
    );
    expect(source.indexOf("<A2B1GrammarVideoCard level={level} day={day} />")).toBeLessThan(
      source.indexOf("<B1Days12To17LearningUpgrade day={numericDay} />"),
    );
  });
