...
function sessionPreference(session, canonicalClassId = "") {
  let score = 0;
  if (String(session.classId || "").trim() === String(canonicalClassId || "").trim()) score += 8;
  if (String(session.classRecordId || "").trim() === String(canonicalClassId || "").trim()) score += 4;
  if (normalizeCurriculumIds(session).length) score += 2;
  if (String(session.topic || "").trim()) score += 1;
  return score;
}

function dedupeSessions(sessions = [], canonicalClassId = "") {
  const byMoment = new Map();
  [...sessions]
    .sort((a, b) => sessionTime(a, "startsAt") - sessionTime(b, "startsAt"))
    .forEach((session) => {
      const startsAt = sessionTime(session, "startsAt");
      const key = startsAt ? `time:${startsAt}` : `id:${session.id}`;
      const existing = byMoment.get(key);
      if (!existing || sessionPreference(session, canonicalClassId) > sessionPreference(existing, canonicalClassId)) {
...
export function subscribeCanonicalLiveClass({ classId, className, slug, onChange, onUnavailable, onError }) {
  ...
  const rebuildSessions = () => {
    currentSessions = dedupeSessions([...sessionBuckets.values()].flat(), currentClass?.id);
  };
  ...
  (async () => {
    try {
      ...
      sessionLookups(klass).forEach(({ field, identifier }) => {
