...
const addUnique = (items, value) => { if (value && !items.includes(value)) items.push(value); };

const grammarRulesFor = (focus = "") => {
  const f = String(focus).toLowerCase();
  const rules = [];
  if (f.includes("indem") || f.includes("dadurch")) addUnique(rules, "Mit indem / dadurch, dass erklärst du eine Methode. Im Nebensatz steht das konjugierte Verb am Ende.");
  if (f.includes("um ... zu") || f.includes("damit")) addUnique(rules, "um ... zu nutzt du meist bei gleichem Subjekt; damit erlaubt ein eigenes Subjekt. Im damit-Satz steht das Verb am Ende.");
  if (f.includes("wodurch") || f.includes("sodass")) addUnique(rules, "wodurch und sodass zeigen eine Folge. Im Nebensatz steht das konjugierte Verb am Ende.");
  if (f.includes("passiv")) addUnique(rules, "Passiv: werden + Partizip II. Modalpassiv: Modalverb + Partizip II + werden am Satzende.");
  if (f.includes("nominalisierung")) addUnique(rules, "Nominalisierung macht Aussagen formeller: fördern → die Förderung, auswerten → die Auswertung, anerkennen → die Anerkennung.");
  if (f.includes("relativ")) addUnique(rules, "Im Relativsatz richtet sich der Kasus des Relativpronomens nach seiner Funktion oder Präposition; das konjugierte Verb steht am Ende.");
  if (f.includes("je ... desto")) addUnique(rules, "je ... desto verbindet zwei Entwicklungen: Je + Komparativ + Nebensatz, desto + Komparativ + Hauptsatz.");
  if (f.includes("obwohl") || f.includes("obgleich")) addUnique(rules, "obwohl / obgleich räumt einen Gegensatz ein; das konjugierte Verb steht im Nebensatz am Ende.");
  if (f.includes("trotz")) addUnique(rules, "trotz steht vor einem Nomen und drückt Einräumung aus; obwohl verbindet dagegen einen ganzen Nebensatz.");
  if (f.includes("ohne ... zu") || f.includes("statt ... zu") || f.includes("anstatt")) addUnique(rules, "ohne ... zu / (an)statt ... zu verbinden Handlungen mit demselben Subjekt; der Infinitiv mit zu steht am Ende.");
  if (f.includes("nicht nur") || f.includes("sowohl")) addUnique(rules, "Zweiteilige Konnektoren verbinden parallele Informationen: nicht nur ... sondern auch / sowohl ... als auch.");
  if (f.includes("während") || f.includes("wohingegen")) addUnique(rules, "während / wohingegen eignen sich für direkte Gegenüberstellungen; im Nebensatz steht das Verb am Ende.");
  if (f.includes("konjunktiv ii")) addUnique(rules, "Konjunktiv II mit könnte, würde, wäre und sollte macht Vorschläge, Möglichkeiten und hypothetische Aussagen differenzierter.");
  if (f.includes("falls") || f.includes("sofern")) addUnique(rules, "falls / sofern formulieren Bedingungen; das konjugierte Verb steht im Nebensatz am Ende.");
  if (f.includes("zwar")) addUnique(rules, "zwar ... jedoch verbindet Einräumung und Gegenargument und eignet sich für ausgewogene B2-Argumente.");
  if (f.includes("laut") || f.includes("zufolge") || f.includes("angaben")) addUnique(rules, "Kennzeichne fremde Informationen mit laut ..., ... zufolge oder nach Angaben von ... .");
  if (f.includes("indirekte rede")) addUnique(rules, "Trenne fremde Aussagen von deiner eigenen Position, z. B. mit dass-Sätzen, Quellenangaben oder – wenn passend – Konjunktiv I.");
  if (f.includes("aufgrund")) addUnique(rules, "aufgrund nennt eine Ursache, trotz eine Einräumung; beide stehen in formeller Sprache häufig mit Genitiv.");
  if (f.includes("review") || f.includes("verknüpfungen")) addUnique(rules, "Wähle die Struktur nach Funktion: Ursache, Folge, Gegensatz, Einräumung, Ziel, Methode oder Bedingung.");
...
const keywordsFor = (alignment) => [...new Set(String(alignment.lessonTopic || alignment.title || "")
  .replace(/[–—,.:;!?()]/g, " ")
  .split(/\s+/)
  .filter((word) => word.length > 5))].slice(0, 6);
