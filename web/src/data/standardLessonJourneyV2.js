...
import b2Day3QuestionWritingBuilder from "./writingQuestionBuilders/b2Day3OeffentlichesPrivatesLeben";
...
import c1Day5QuestionWritingBuilder from "./writingQuestionBuilders/c1Day5BeruflicheEntwicklung";
...
const SPECIAL_WRITING_CONFIGS = {
  ...
  "B2-3": b2Day3QuestionWritingBuilder,
  ...
  "C1-5": c1Day5QuestionWritingBuilder,
};
...
export const getStandardWritingConfig = (lesson = {}) => {
  ...
  const prompts = [
    ...
    [`Welches konkrete Beispiel erklärt das Thema „${title}“ besonders gut?`, "Nutze eine Situation aus Alltag, Arbeit, Schule, Familie oder Gesellschaft."],
    [isC1 ? `Welcher Einwand oder welche alternative Perspektive muss bei „${title}“ berücksichtigt werden?` : `Welche andere Möglichkeit, Lösung oder Gegenposition gibt es bei „${title}“?`, "Zeige, dass du das Thema nicht nur aus einer Perspektive betrachtest."],
    [`Welche ausgewogene Lösung oder Schlussposition vertrittst du beim Thema „${title}“?`, isC1 ? "Verbinde eine tragfähige Lösung mit einem differenzierten abschließenden Urteil." : "Nenne eine sinnvolle Lösung und fasse deine Position klar zusammen."],
  ];
...
const parsePlanItem = (item = "", index = 0) => {
  ...
  return rest.length
    ? { title: rawTitle.trim(), prompt: rest.join(":").trim() }
    : { title: `Punkt ${index + 1}`, prompt: String(item).trim() };
};
...
export const getStandardLessonStorageKey = (lesson = {}, suffix = "progress") =>
  `falowen:${normalizeLevel(lesson.level || "B1").toLowerCase()}:day${Number(lesson.day || 0)}:standard-journey:${suffix}`;

export const getStandardWritingCloudField = (lesson = {}) =>
  `standard${normalizeLevel(lesson.level || "B1")}Day${Number(lesson.day || 0)}GuidedWriting`;
